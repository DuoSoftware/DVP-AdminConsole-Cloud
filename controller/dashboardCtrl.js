...
mainApp.controller("dashboardCtrl", function (
  ...
  subscribeServices.SubscribeConnection("dashboard_socket", function (
    isConnected
  ) {
    if (isConnected) {
  ...
  subscribeServices.subscribeDashboard("dashboard", function (event) {
    if (
      ...
      ((ShareData.BusinessUnit.toLowerCase() === "all" &&
        ...
        (ShareData.BusinessUnit.toLowerCase() === "all" &&
          event.From === "ArdsMonitoringService") ||
        event.Message.businessUnit.toLowerCase() ===
      ...
      switch (event.roomName) {
        case "ARDS:ResourceStatus":
          if (event.Message) {
            userImageList.getAvatarByUserName(event.Message.userName, function (
              res
            ) {
          ...
          if (event.Message) {
            ...
            if (event.Message.queueDetail.QueueInfo.TotalQueued > 0) {
              item.presentage = Math.round(
                (event.Message.queueDetail.QueueInfo.TotalAnswered /
                  event.Message.queueDetail.QueueInfo.TotalQueued) *
                  100
              );
            }
            ...
            if (!$scope.queues[item.id]) {
              ...
              dashboardService.getQueueRecordDetails(queueID).then(
                function (resQueue) {
                  if (
                    resQueue.data &&
                    resQueue.data.IsSuccess &&
                    resQueue.data.Result
                  ) {
        ...
        case "ARDS:break_exceeded":
          if (event.Message) {
            var agent = $filter("filter")($scope.StatusList.BreakProfile, {
        ...
        case "ARDS:freeze_exceeded":
          if (event.Message) {
            var agent = $filter("filter")($scope.StatusList.AfterWorkProfile, {
              resourceId: event.Message.ResourceId,
            });
            if (agent && agent.length > 0) {
      ...
      console.info(
        "Subscribe Dashboard Event Receive For Invalid Business Unit"
      );
    }
  });
  ...
  var ServerHandler = (function () {
    ...
    return {
      ...
      getAllQueued: function () {
        var deferred = $q.defer();
        dashboardService.GetAllQueued().then(
          function (response) {
            if (response && response.length > 0) {
              response.pop();
              var max = 0;
      ...
      getAllBriged: function () {
        var deferred = $q.defer();
        dashboardService.GetAllBriged().then(
          function (response) {
            if (response && response.length > 0) {
              response.pop();
      ...
      getTotalQueueHit: function () {
        var deferred = $q.defer();
        dashboardService.GetTotalQueueHit().then(
          function (response) {
            if (response && response.length > 0) {
              response.pop();
              var max = 0;
              $scope.dataSetChannels[0].data = response.map(function (
                c,
                index
              ) {
                var item = [];
      ...
      getTotalCall: function () {
        dashboardService.GetTotalCalls("inbound", null).then(
          function (responseInb) {
        ...
        dashboardService.GetTotalCalls("outbound", null).then(
          function (responseOutb) {
      ...
      getTotalQueued: function () {
        dashboardService.GetTotalQueued().then(
          function (response) {
      ...
      getTotalQueueAnswered: function () {
        dashboardService.GetTotalQueueAnswered().then(
          function (response) {
      ...
      getCurrentWaiting: function () {
        dashboardService.GetCurrentWaiting().then(
          function (response) {
      ...
      getTotalBriged: function () {
        dashboardService.GetTotalBriged().then(
          function (response) {
      ...
      getTotalOnGoing: function () {
        dashboardService.getCurrentBridgedCalls("inbound").then(
          function (response) {
        ...
        dashboardService.getCurrentBridgedCalls("outbound").then(
          function (response) {
    ...
    dashboardService.GetProfileDetails().then(function (response) {
      ...
      if (response.length > 0) {
        for (var i = 0; i < response.length; i++) {
          ...
          if (
            (response[i] &&
              response[i].BusinessUnit &&
              response[i].BusinessUnit.toLowerCase()) ===
              ShareData.BusinessUnit.toLowerCase() ||
            ShareData.BusinessUnit.toLowerCase() === "all"
          ) {
            var profile = {
            ...
            userImageList.getAvatarByUserName(profile.resourceName, function (
              res
            ) {
            ...
            if (
              response[i].ConcurrencyInfo &&
              response[i].ConcurrencyInfo.length > 0
            ) {
              response[i].ConcurrencyInfo.forEach(function (concurrency) {
                if (
                  ...
                  if (reservedDate == "") {
                    ...
                    profile.LastReservedTime = moment(reservedDate).format(
                      "h:mm a"
                    );
                  }
                }
